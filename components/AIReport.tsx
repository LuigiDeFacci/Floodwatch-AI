...
const AIReport: React.FC<AIReportProps> = ({ 
  ...
  if (report) {
    return (
      <div className="relative overflow-hidden bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl p-5 rounded-2xl border border-indigo-100 dark:border-indigo-800 shadow-sm animate-fade-in transition-colors duration-300">
  ...
  return (
    <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl p-1 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-colors duration-300 h-full">
      <button
        onClick={onGenerate}
        disabled={loading}
        className="w-full h-full group relative overflow-hidden rounded-xl bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 transition-all duration-300 p-4 flex items-center justify-center gap-3 border border-dashed border-slate-300 dark:border-slate-600 hover:border-indigo-300 dark:hover:border-indigo-500"
      >
        {loading ? (
